...
if(isset($_GET['delete_all'])){
    mysqli_query($conn, "DELETE FROM `tbl_wishlist` WHERE user_id = '$user_id'") or die('query failed');
    header('location:wishlist.php');
}
...
<body>
    ...
    <section class="wishlist">
        ...
        <div class="box-container">
            ...
            <?php
        $grand_total = 0;
        $select_wishlist = mysqli_query($conn, "SELECT * FROM `tbl_wishlist` WHERE user_id = '$user_id'") or die('query failed');
    ...
    ?>
            <form action="" method="POST" class="box">
                ...
                <div class="product_offer">Offer <?php echo $fetch_wishlist['product_offer']; ?> % off*</div>
                <input type="hidden" name="product_id" value="<?php echo $fetch_wishlist['pid']; ?>">
                <input type="hidden" name="product_name" value="<?php echo $fetch_wishlist['name']; ?>">
                <input type="hidden" name="product_price" value="<?php echo $fetch_wishlist['price']; ?>">
                <input type="hidden" name="product_offer" value="<?php echo $fetch_wishlist['product_offer']; ?>">
                <input type="hidden" name="product_image" value="<?php echo $fetch_wishlist['image']; ?>">
                <input type="submit" value="add to cart" name="add_to_cart" class="btn">

            </form>
            <?php $offer = ($fetch_wishlist['price'] / 100) * ($fetch_wishlist['product_offer']) ; ?>
            <?php $total =  $fetch_wishlist['price'] ?>
            <?php $sub_total = $total - $offer  ;
