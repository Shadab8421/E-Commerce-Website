    ...
    <section class="display-order">
        ...
        if(mysqli_num_rows($select_cart) > 0){
            while($fetch_cart = mysqli_fetch_assoc($select_cart)){
             $offer = ($fetch_cart['price'] / 100) * ($fetch_cart['product_offer']) ; 
             $total =  ($fetch_cart['price'] * $fetch_cart['quantity']); 
               
            $total_price = ($total - $offer * $fetch_cart['quantity']);
            $grand_total += $total_price;
    ?>
        <p> <?php echo $fetch_cart['name'] ?>
            <span>(<?php echo '₹'.$fetch_cart['price'] - $offer.'/-'.' x'.$fetch_cart['quantity'], ',  ' ,'Size:'.$fetch_cart['p_size']  ?>)</span>
        </p>
        <?php
        }
        }else{
