...
if(isset($_POST['update_product'])){
   ...
   mysqli_query($conn, "UPDATE `tbl_products` SET name = '$name', product_details = '$details', price = '$price' WHERE id = '$update_p_id'") or die('query failed');
   ...
   if(!empty($image_01)){
      if($image_size_01 > 2000000){
         ...
         mysqli_query($conn, "UPDATE `tbl_products` SET image = '$image_01' WHERE id = '$update_p_id'") or die('query failed');
...
<body>
   ...
   $select_products = mysqli_query($conn, "SELECT * FROM `tbl_products` WHERE id = '$update_id'") or die('query failed');
   if(mysqli_num_rows($select_products) > 0){
      while($fetch_products = mysqli_fetch_assoc($select_products)){
?>

        <form action="" method="post" enctype="multipart/form-data">
            <img src="uploaded_img/<?php echo $fetch_products['image_01']; ?>" class="image" alt="">
            <input type="hidden" value="<?php echo $fetch_products['id']; ?>" name="update_p_id">
            <input type="hidden" value="<?php echo $fetch_products['image_01']; ?>" name="update_p_image">
            ...
            <textarea name="details" class="box" required placeholder="update product details" cols="30"
                rows="10"><?php echo $fetch_products['product_details']; ?></textarea>
